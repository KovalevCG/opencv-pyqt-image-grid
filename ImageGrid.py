...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.transfer_button.setIcon(QtGui.QIcon("./img/SVG/edit.svg"))
        self.transfer_button.setIconSize(QtCore.QSize(20, 20))
        self.transfer_button.setToolTip("Open Editor")
        ...
        self.save_as_button.setIcon(QtGui.QIcon("./img/SVG/save.svg"))
...
class Opencv:
    ...
    def main_loop(self, num_of_c, num_of_r):
        ...
        # img_save = cv2.imread("save-opencv.png", cv2.IMREAD_UNCHANGED)
        # img_save_mask = img_save[4]
        ...
        # --- Save As Button ---
        # Read Image (img_save)
        img_save = cv2.imread("./img/save-opencv.png")
        img_save = cv2.resize(img_save, (40, 40))
        # img_save Shape
        img_save_rows, img_save_cols, _ = img_save.shape
        # img_save Masks
        img_save_mask = img_save[:, :, 0]
        img_save_mask_inverted = cv2.bitwise_not(img_save_mask)
        # White Image with img_save Shape
        img_save_ones = np.ones((img_save_rows, img_save_cols, 3), dtype='uint8')
        img_save_white = img_save_ones * 128
        ...
        while not self.close_cv:
            ...
            if self.mouse_on_type == "save_button":
                img_save_white[5:35, 5:35] = self.highlight_color
            else:
                img_save_white[:, :] = (128, 128, 128)

            stack_rows, stack_cols, channel = stack.shape

            roi = stack[0:img_save_rows, (stack_cols - img_save_cols):]

            roi_mask = cv2.bitwise_and(roi, roi, mask=img_save_mask)
            logo_white_mask = cv2.bitwise_and(img_save_white, img_save_white, mask=img_save_mask_inverted)
            roi_final = cv2.add(roi_mask, logo_white_mask)


            stack[0:img_save_rows, (stack_cols - img_save_cols):] = roi_final
    ...
    def mouse_event(self, event, x, y, flags, params):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            elif self.mouse_on_type == "save_button":
    ...
    def mouse_tile_position(self, x, y):
        ...
        # Save Button
        if x > width_total - 40 and y < 40:
            self.mouse_on_type = "save_button"
            return
        # Right Border
        if x > width_total - 9:
            self.border_time = time.time()
